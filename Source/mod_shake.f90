...
!      !$omp& shared(shakeatom,mass,position,ff)
 ...
 !     !$omp end parallel do
    ...
    subroutine shake2(i0,i1,bond1,dtsq_factor)
      ...
      !---positiong of atoms with unconstained update, 
      x0 = position(i0)%x + dt*v(i0)%x + dtsq_factor*invmass0*ff(i0)%x
      y0 = position(i0)%y + dt*v(i0)%y + dtsq_factor*invmass0*ff(i0)%y
      z0 = position(i0)%z + dt*v(i0)%z + dtsq_factor*invmass0*ff(i0)%z

      x1 = position(i1)%x + dt*v(i1)%x + dtsq_factor*invmass1*ff(i1)%x
      y1 = position(i1)%y + dt*v(i1)%y + dtsq_factor*invmass1*ff(i1)%y
      z1 = position(i1)%z + dt*v(i1)%z + dtsq_factor*invmass1*ff(i1)%z
      ...
      if (determ < 0.0) then
        ...
        determ = 0.0
        print*,b,a,c
        print*,position(i0)%x,position(i0)%y,position(i0)%z,position(i0)%type
        print*,position(i1)%x,position(i1)%y,position(i1)%z,position(i1)%type
        stop
      endif
      ...
      ff(i0)%x = ff(i0)%x + lamda*r01(1)
      ff(i0)%y = ff(i0)%y + lamda*r01(2)
      ff(i0)%z = ff(i0)%z + lamda*r01(3)
      
      ff(i1)%x = ff(i1)%x - lamda*r01(1)
      ff(i1)%y = ff(i1)%y - lamda*r01(2)
      ff(i1)%z = ff(i1)%z - lamda*r01(3)
    ...
    subroutine shake3(i0,i1,i2,bond1,bond2,dtsq_factor)
      ...
      ff(i0)%x = ff(i0)%x + lamda01*r01(1) + lamda02*r02(1)
      ff(i0)%y = ff(i0)%y + lamda01*r01(2) + lamda02*r02(2)
      ff(i0)%z = ff(i0)%z + lamda01*r01(3) + lamda02*r02(3)
      
      
      ff(i1)%x = ff(i1)%x - lamda01*r01(1)
      ff(i1)%y = ff(i1)%y - lamda01*r01(2)
      ff(i1)%z = ff(i1)%z - lamda01*r01(3)
      
      ff(i2)%x = ff(i2)%x - lamda02*r02(1)
      ff(i2)%y = ff(i2)%y - lamda02*r02(2)
      ff(i2)%z = ff(i2)%z - lamda02*r02(3)
    ...
    subroutine shake4(i0,i1,i2,i3,bond1,bond2,bond3,dtsq_factor)
      ...
      lamda03 = lamda03/dtsq_factor
        ...
        ff(i3)%x = ff(i3)%x - lamda03*r03(1)
        ff(i3)%y = ff(i3)%y - lamda03*r03(2)
        ff(i3)%z = ff(i3)%z - lamda03*r03(3)


       ! if(i1.eq.8.or.i2.eq.8.or.i3.eq.8)then
       !    print*,'lets check the forces'
       !    print*,'what atoms are here',i0,i1,i2,i3
       !    print*,'what are the bond distances',bond1*rsig,bond2*rsig,bond3*rsig
       !    print*,'whats force contribution', lamda01*r01(1),lamda01*r01(2), lamda01*r01(3)
       !    print*,'whats force contribution', lamda02*r02(2),lamda02*r02(2), lamda01*r01(3)
       !    print*,'whats force contribution', lamda03*r03(1),lamda03*r03(2), lamda03*r03(3)
       !    print*
       !    print*
    ...
    subroutine shake3angle(i0,i1,i2,bond1,bond2,bond12,dtsq_factor)
     ...
     !update forces if atom is owned by this processor
      ...
      ff(i1)%x = ff(i1)%x - lamda01*r01(1) + lamda12*r12(1)
      ff(i1)%y = ff(i1)%y - lamda01*r01(2) + lamda12*r12(2)
      ff(i1)%z = ff(i1)%z - lamda01*r01(3) + lamda12*r12(3)
     
      ff(i2)%x = ff(i2)%x - lamda02*r02(1) - lamda12*r12(1)
      ff(i2)%y = ff(i2)%y - lamda02*r02(2) - lamda12*r12(2)
      ff(i2)%z = ff(i2)%z - lamda02*r02(3) - lamda12*r12(3)
...
!      !$omp parallel do default(firstprivate),&
!      !$omp& shared(mass,xshake,position,v,ff)
      do i = 1,np
